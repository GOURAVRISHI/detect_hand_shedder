...
if __name__ == '__main__':
    ...
    #vs = cv2.VideoCapture('rtsp://192.168.1.64')   # Configure IP of IP camera
    vs = VideoStream(0).start()             # 0 for laptop camera and 1 for external camera
    #Oriendtation of machine
    Orientation= 'bt' # bottom to top
    # Orientation=input("Enter the orientation of hand progression ~ lr,rl,bt,tb :")  # lr = left to right, rl= right to left, tb = top to bottom, bt= bottom to top

    #For Machine (Line percentage for Machine Border Line)
    #Line_Perc1=float(input("Enter the percent of screen the line of machine :"))
    Line_Perc1=float(15)

    #For Safety (Line percentage for Safety Border Line)
    ...
    cv2.namedWindow('Detection', cv2.WINDOW_NORMAL)      # pop up window name
    count=0
    lic = ''
    cv2.namedWindow('Detection', cv2.WINDOW_NORMAL)
    try:
        while True:
        # Reading Frame and process
            count +=1
            frame = vs.read()
